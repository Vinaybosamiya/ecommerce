...
const playMusic = (track, pause = false) => {
    ...
    if (!track) {
        console.warn("Track is undefined, skipping play.");
        return;
    }
    if (!pause) {
   ...
   let array = Array.from(anchor)
        ...
        Array.from(document.getElementsByClassName("card")).forEach(e => {
            ...
            e.addEventListener("click", async items => {
                ...
                playMusic(songs[0]) // that is automatic play music 
                // this is not play automatic music
                // if (songs.length > 0) {
                //     playMusic(songs[0], true);
                // } else {
                //     console.error("No songs found in the folder:", curentFolder);
                // }
